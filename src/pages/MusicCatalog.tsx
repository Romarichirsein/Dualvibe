...
export default function MusicCatalog({ lang, searchQuery = "", addToCart, openCart }: MusicCatalogProps) {
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16 relative">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-lime-500/10 blur-[100px] rounded-full" />
          <motion.h1 
          ...
          <motion.div 
            ...
            className="mt-8 inline-flex items-center gap-2 glass px-4 py-2 rounded-full text-xs font-bold text-lime-500"
          >
            ...
            <button
              ...
              className={`px-6 py-3 rounded-full text-sm font-bold transition-all ${
                activeCategory === category
                  ? 'bg-gradient-to-r from-lime-500 to-sky-700 text-white shadow-lg shadow-lime-500/25'
                  : 'glass text-slate-600 dark:text-slate-200 hover:bg-lime-500/10'
              }`}
            >
          ...
          {filteredMusic.map((track) => (
            <motion.div
              ...
              className={`glass rounded-[2rem] p-6 transition-all border-2 ${
                playingTrack === track.id ? 'border-lime-500' : 'border-transparent hover:border-lime-500/30'
              }`}
            >
              ...
              <div className="mt-6 pt-6 border-t border-slate-200 dark:border-white/10 text-center">
                <button 
                  ...
                  className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-lime-500 transition-colors"
                >
          ...
          >
            ...
            >
              <button 
                ...
                className="absolute top-6 right-6 w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center hover:bg-lime-500 hover:text-white transition-colors"
              >
                <X className="w-5 h-5" />
              </button>

              <h2 className="text-2xl font-bold mb-2 text-black dark:text-white">{t.formTitle}</h2>
              <p className="text-lime-500 font-bold mb-8 text-sm line-clamp-1">{selectedTrack.title}</p>
    ...
    <section className="py-20 border-t border-white/10 overflow-hidden relative">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-sky-700/10 blur-[120px] rounded-full" />
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-br from-sky-700/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
          <div className="inline-flex p-4 bg-sky-600/10 rounded-2xl text-sky-600 mb-6 animate-float">
          ...
          <Link to="/studio">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-10 py-4 bg-gradient-to-r from-sky-700 to-sky-900 text-white rounded-2xl font-bold flex items-center justify-center gap-3 mx-auto shadow-xl shadow-sky-700/20"
            >
