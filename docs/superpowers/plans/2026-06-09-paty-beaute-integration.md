# Paty Beauté Integration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Integrate the dedicated page for the partner salon "Paty Beauté" into the Dualvibe project, complete with route, promotion in the services list, logo and image assets, and an interactive option modal.

**Architecture:** We will copy the assets to `public/products/paty-beaute/`, configure route mapping in `src/App.tsx`, promote the salon in `src/pages/Services.tsx`, and build a rich multi-language catalog page at `src/pages/PatyBeaute.tsx` with an interactive booking modal for variations.

**Tech Stack:** React, React Router, Tailwind CSS, Lucide icons, motion/react (framer-motion), Supabase client.
...
**Files:**
- Create: `public/products/paty-beaute/` (Folder)
- Copy files from: `Dual vibe products/Services/paty beaute/`

- [ ] **Step 1: Create the destination directory and copy files**

Run the following PowerShell commands to create the directory and copy the contents:
```powershell
New-Item -ItemType Directory -Force -Path "public/products/paty-beaute"
Copy-Item -Path "Dual vibe products/Services/paty beaute/*" -Destination "public/products/paty-beaute" -Recurse -Force
```
Expected: The files are copied. Specifically, `Logo paty beaute.png` should be copied.

- [ ] **Step 2: Rename the logo file for clean reference**

Run PowerShell command:
```powershell
Rename-Item -Path "public/products/paty-beaute/Logo paty beaute.png" -NewName "logo.png" -Force
```
Expected: File `public/products/paty-beaute/logo.png` exists.

- [ ] **Step 3: Commit copied assets**

Run:
```bash
git add public/products/paty-beaute
git commit -m "assets: copy and configure Paty Beauté images and logo"
```

---

### Task 2: Configure Routing in App.tsx

**Files:**
- Modify: `src/App.tsx`

- [ ] **Step 1: Add Lazy Import for PatyBeaute**

Search for `const GabNails = React.lazy` in [App.tsx](file:///c:/Users/COMPUTER%20STORES/Desktop/Dualvibe/src/App.tsx#L29) and add the lazy import for PatyBeaute right below it:
```typescript
const PatyBeaute = React.lazy(() => import('./pages/PatyBeaute'));
```

- [ ] **Step 2: Add Path Check in DynamicRouteHandler**

Find `if (location.pathname === '/gabs-nails')` in [App.tsx](file:///c:/Users/COMPUTER%20STORES/Desktop/Dualvibe/src/App.tsx#L46-L54) and add the path check for `/paty-beaute` right below or above it:
```typescript
  if (location.pathname === '/paty-beaute') {
    return (
...
```bash
git add src/App.tsx
git commit -m "router: add lazy loading and dynamic routing for /paty-beaute"
```

---

### Task 3: Create PatyBeaute Catalog Page Component

**Files:**
- Create: `src/pages/PatyBeaute.tsx`

- [ ] **Step 1: Write the PatyBeaute page component**

Create the file `src/pages/PatyBeaute.tsx` containing the full React component, bilingual text, structured catalog products (from spec), filters, and the booking options modal.
```typescript
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { ArrowRight, Sparkles, MapPin, Clock, ExternalLink, X, Check, Calendar, Phone } from 'lucide-react';
    ...
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen">
      ...
      >
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm opacity-85 leading-relaxed">
          ...
          <li className="flex items-start gap-2.5">
            <Check className="w-5 h-5 text-green-400 shrink-0 mt-0.5" />
            <span>{lang === 'fr' ? "Un service de qualité supérieure avec une styliste à l'écoute." : "Top-tier quality service with a attentive and patient stylist."}</span>
          </li>
        </ul>
      </motion.div>
...
Run:
```bash
git add src/pages/PatyBeaute.tsx
git commit -m "feat: implement PatyBeaute salon page and option modal"
```

---

### Task 4: Promote Paty Beauté in Services Page

**Files:**
- Modify: `src/pages/Services.tsx`

- [ ] **Step 1: Insert Paty Beauté card into Services.tsx**

Locate the promotional card of CHEM'S or Gab's Nails in [Services.tsx](file:///c:/Users/COMPUTER%20STORES/Desktop/Dualvibe/src/pages/Services.tsx#L117-L151) and add a promotional section for Paty Beauté right above or below it:
```typescript
...
- [ ] **Step 2: Commit Services.tsx Promotion Card**

Run:
```bash
git add src/pages/Services.tsx
git commit -m "feat: add promotional card for Paty Beauté on Services page"
```

---

### Task 5: Compilation and Local Verification

**Files:**
- None (verification task)

- [ ] **Step 1: Check code compilation**

Run typescript check or run a dev build to verify no errors.
Run:
```powershell
npm run build
```
Expected: The build completes with exit code 0 and no TypeScript compilation errors.
